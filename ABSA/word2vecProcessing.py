...
def trainingForWords(texts,fnum,filepath,sg,hs):
    #设定词向量参数
    sentences=[str(text).lower().split(' ') for text in texts]
    print('分词完成，开始训练')
...
def trainingEmbedding(vector_len=150,d_type='re',add_extra=False):
    ...
    train_corpus=preProcessing.loadXML('data/origin/ABSA_2014_origin/%s_Train_v2.xml'%d_name)
    test_corpus=preProcessing.loadXML('data/origin//ABSA_2014_origin/%s_Test_Data_PhaseA.xml'%d_name)
    print('数据集合并完成')
